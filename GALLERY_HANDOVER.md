...
| `myllmImmersive(on, {keepAwake})` | hide MyLLM's title + status bars | **iOS 5.6.1+.** For a view meant to fill the display (a player, a dock, a kiosk clock): the page then reaches every edge, so pad with `env(safe-area-inset-*)`. `keepAwake:true` stops the screen sleeping while it lasts. **The page must offer its own way back** (call it again with `false`); MyLLM restores the bars whenever the app is left, so re-ask on `visibilitychange`. `position:fixed; inset:0` alone can never cover the title bar — it is native. Used by Internet Radio. |
| `myllmStreamTitle(url)` | what a radio stream says is playing | **iOS 5.6.1+.** `→Promise<{title, station, metadata}>`. Reads the stream's in-band ICY metadata (which a page cannot: the `Icy-MetaData` header is CORS-blocked on most stations) over its own short connection and hangs up at the first title. `title:""` with `metadata:true` = blank right now (ad break), with `metadata:false` = this stream has none (HLS, many talk stations) so stop asking. Poll ~20 s while playing. Titles are raw — stations put their name, URLs and ad markers in there; clean before showing (see `cleanTitle` in `internet-radio.html`). Known limit: stations that play every new connection an ad pre-roll (KEXP) answer blank to a short read, always — back off rather than keep asking. |
